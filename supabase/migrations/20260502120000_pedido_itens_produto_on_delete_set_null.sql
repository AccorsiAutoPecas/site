-- Excluir produto com histórico em pedidos: desvincula FK e mantém snapshots na linha do item.
...
comment on column public.pedido_itens.produto_id is
  'Referência ao catálogo; NULL se o produto foi removido (titulo_snapshot e cod_produto_snapshot preservam a compra).';

-- Baixa de estoque: ignora itens já desvinculados (sem linha em public.produtos).
...
begin
  ...
  if p_mp_status = 'approved' then
    ...
    loop
      if r_item.produto_id is null then
        continue;
      end if;
...
comment on function public.mercadopago_aplicar_estado_pagamento is
  'Aplica notificação de pagamento MP: baixa estoque só em approved e aguardando_pagamento; ignora itens sem produto_id; idempotente para o mesmo payment_id.';
